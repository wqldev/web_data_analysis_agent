...
CONFIG_PATH = ROOT / "data" / "cursor_config.json"
...
def load_config() -> dict[str, Any]:
    ...
    env_model = os.getenv("CURSOR_MODEL", "").strip()
...
def save_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "model": str(merged.get("model") or "composer-2.5").strip(),
    }
...
def public_config(config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        "model": cfg.get("model", "composer-2.5"),
...
def get_model() -> str:
    return str(load_config().get("model") or "composer-2.5").strip()
